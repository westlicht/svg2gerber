...
class Transformable:
    ...
    def getTransformations(self, elt):
        ...
        transforms = re.findall("|".join([x + "[^)]*\)" for x in svg_transforms]), t)
...
class Group(Transformable):
    ...
    def __repr__(self):
        return (
            "<Group " + self.id + " ({})".format(self.name) + ">: " + repr(self.items)
        )
...
class Matrix:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Matrix):
            ...
            e = (
                self.vect[0] * other.vect[4]
                + self.vect[2] * other.vect[5]
                + self.vect[4]
            )
            f = (
                self.vect[1] * other.vect[4]
                + self.vect[3] * other.vect[5]
                + self.vect[5]
            )
...
class Path(Transformable):
    ...
    def segments(self, precision=0):
        ...
        for moveTo, group in itertools.groupby(
            self.items, lambda x: isinstance(x, MoveTo)
        ):
...
class Ellipse(Transformable):
    ...
    def __init__(self, elt=None):
        ...
        if elt is not None:
            self.center = Point(
                self.xlength(elt.get("cx")), self.ylength(elt.get("cy"))
            )
...
class Rect(Transformable):
    ...
    def __init__(self, elt=None):
        ...
        if elt is not None:
            ...
            self.P2 = Point(
                self.P1.x + self.xlength(elt.get("width")),
